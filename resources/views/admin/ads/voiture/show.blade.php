...
@section('content')
    @section('activeVoiture') active @endsection
    <div class="card card-custom col-lg-10 mx-auto  example example-compact">
        @section('titre') View {{ $voiture->title }} @endsection
        ...
        @endif
        <form action="{{ route('ads.voiture.update',['id'=>$voiture->id]) }}" method="post">
            @csrf
            @method('patch')
                            ...
                            <select name="user_id" class="form-control select2 @error('user_id')
                                ...
                                @foreach ($user as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->id === $voiture->user_id ? 'selected' : '' }}>
                                        {{ $item->username }}
                                            ...
                                            @foreach ($item->categories as $category)
                                                ...
                                                @foreach ($category->type as $type)
                                                    <option value="{{ $type->id }}"
                                                        {{ $type->id === $voiture->type_id ? 'selected' : '' }}>
                                                        {{ $type->name }}</option>
                                                @endforeach
                                        ...
                                        <optgroup label="{{ $item->name }}">
                                            @foreach ($item->categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ $category->id === $voiture->category_id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="form-group col-lg-6">
                            ...
                            <input type="date" name="created_at"
                                ...
                                value="{{ $voiture->created_at->format('Y-m-d') }}" disabled>
                            @error('created_at')
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group col-lg-6">
                            ...
                            <input type="date" name="expired_at"
                                ...
                                value="{{ $voiture->expired_at->format('Y-m-d') }}" disabled>
                            @error('expired_at')
                                <div class="invalid-feedback">
                    ...
                    <div class="row">
                        <div class="form-group col-lg-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" name="description"
                                id="kt_autosize_1" rows="3">{{ $voiture->description }}</textarea>
                        </div>
                    </div>
                                ...
                                <span class="switch switch-outline switch-icon switch-success">
                                    <label>
                                        ...
                                        <input type="checkbox" name="statu" value="active"
                                            {{ $voiture->statu === 'active' ? 'checked' : '' }} class="disables">
                                        <span></span>
                                    </label>
                                </span>
                            </div>
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <a href="{{ route('ads.voiture') }}" class="btn btn-dark font-weight-bold mr-2">Back</a>
                    </div>
                </div>
            </div>
        </form>
