...
@section('content')
    @section('activeImmobilier') active @endsection
    <div class="card card-custom col-lg-10 mx-auto  example example-compact">
        @section('titre')View {{ $immobilier->title }}  @endsection 
        ...
        @endif
        <form action="{{ route('ads.immobilier.update', ['id' => $immobilier->id]) }}" method="post">
            @csrf
            @method('patch')
                ...
                <div class="card-body">
                            ...
                            <select name="user_id" class="form-control select2 @error('user_id')
                                ...
                                @foreach ($user as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->id === $immobilier->user_id ? 'selected' : '' }}>
                                        {{ $item->username }}
                                            ...
                                            @foreach ($item->categories as $category)
                                                ...
                                                @foreach ($category->type as $type)
                                                    <option value="{{ $type->id }}"
                                                        {{ $type->id === $immobilier->type_id ? 'selected' : '' }}>
                                                        {{ $type->name }}</option>
                                                @endforeach
                                        ...
                                        <optgroup label="{{ $item->name }}">
                                            @foreach ($item->categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ $category->id === $immobilier->category_id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="form-group col-lg-6">
                            ...
                            <input type="date" name="created_at"
                                ...
                                value="{{ $immobilier->created_at->format('Y-m-d') }}" disabled>
                            @error('created_at')
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group col-lg-6">
                            ...
                            <input type="date" name="expired_at"
                                ...
                                value="{{ $immobilier->expired_at->format('Y-m-d') }}" disabled>
                            @error('expired_at')
                                <div class="invalid-feedback">
                    ...
                    <div class="row">
                        <div class="form-group col-lg-12">
                            ...
                            <input type="text" name="price" class="form-control @error('price') is-invalid @enderror"
                                value="{{ $immobilier->price }}" disabled>
                            @error('price')
                                <div class="invalid-feedback">
                                    {{ $errors->first('price') }}
                                </div>
                    ...
                    <div class="row">
                        <div class="form-group col-lg-12">
                            ...
                            <textarea class="form-control @error('message') is-invalid @enderror" name="description"
                                id="kt_autosize_1" rows="3">{{ $immobilier->description }}</textarea>
                        </div>
                    </div>
                                ...
                                <span class="switch switch-outline switch-icon switch-success">
                                    <label>
                                        ...
                                        <input type="checkbox" name="statu" value="active"
                                            {{ $immobilier->statu === 'active' ? 'checked' : '' }} class="disables">
                                        <span></span>
                                    </label>
                                </span>
                            </div>
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <a href="{{ route('ads.immobilier') }}" class="btn btn-dark font-weight-bold mr-2">Back</a>
                    </div>
                </div>
            </div>
        </form>
